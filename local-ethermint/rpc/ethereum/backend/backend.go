...
func (e *EVMBackend) GetTransactionByHash(txHash common.Hash) (*types.RPCTransaction, error) {
	...
	fmt.Print("[ethermint] backend.go e.GetEthereumMsgsFromTendermintBlock(resBlock) passed\n")
	fmt.Printf("[ethermint] backend.go msgs: %#v\n", msgs)

	for i := range msgs {
		if msgs[i].Hash == hexTx {
			txIndex = uint64(i)
			fmt.Printf("backend.go txMatch at i:%v, hash: %v\n", i, hexTx)
			break
		}
	}

	msg := msgs[txIndex]
	fmt.Printf("[ethermint] backend.go msg: %#v\n", msg)
...
func (e *EVMBackend) GetEthereumMsgsFromTendermintBlock(block *tmrpctypes.ResultBlock) []*evmtypes.MsgEthereumTx {
	...
	fmt.Print("[ethermint] e.GetEthereumMsgsFromTendermintBlock\n")
	fmt.Printf("[ethermint] e.GetEthereumMsgsFromTendermintBlock block: %#v\n", block)
	...
	for _, tx := range block.Block.Txs {
		...
		for _, msg := range tx.GetMsgs() {
			...
			fmt.Printf("[ethermint] e.GetEthereumMsgsFromTendermintBlock ethMsg hash: %#v\n", hash)
			...
			fmt.Printf("[ethermint] e.GetEthereumMsgsFromTendermintBlock ethTx: %#v\n", ethTx)
